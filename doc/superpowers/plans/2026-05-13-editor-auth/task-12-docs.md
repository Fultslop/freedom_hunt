...
**Files:**
- Create: `doc/auth-guide.md`

This guide has two audiences:
1. **Organizers** — non-technical people who run hunts and need to grant editor access
2. **Self-hosters** — technical people setting up their own instance

---

- [ ] **Step 1: Create `doc/auth-guide.md`**
...
```bash
npx wrangler d1 execute freedom_hunt_auth --file=migrations/001_init.sql
```
...
```bash
npx wrangler d1 execute freedom_hunt_auth \
...
- [ ] **Step 2: Update the devlog**

Add an entry at the top of `doc/devlog/_devlog.md` following the project format:

```
**13/05/2026, Claude**: [FEAT] Editor auth — individual accounts, invite flow, D1 backend.
- Replaced shared admin KV password with individual user accounts in Cloudflare D1
- New endpoints: signup, D1 login, bootstrap promote, invite create/validate/accept, editor user management
- New frontend pages: SignupPage (with GDPR consent), InviteAcceptPage
- Token discrimination: three payload shapes (participant/user/bootstrap) by key presence
- Passwords hashed with PBKDF2-SHA256 via Web Crypto API — no external dependency
- User-facing guide written to doc/auth-guide.md covering organizer and self-hoster flows
```
...
```bash
git add doc/auth-guide.md doc/devlog/_devlog.md
git commit -m "docs: add auth guide for organizers and self-hosters; update devlog"
```
```
