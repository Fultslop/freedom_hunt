# Task 11: Add `src/test/AppForm.test.ts`

**Files:**
- Create: `src/test/AppForm.test.ts`

Tests for `AppForm.svelte` covering: field type rendering, validation, confirmation dialog, `onSuccess` callback, dotted-path output to `onSubmit`, `initialValues`, and `section` heading rendering.

---

- [ ] **Step 1: Create `src/test/AppForm.test.ts`**
...
```
git add src/test/AppForm.test.ts
git commit -m "test: add AppForm.test.ts — unit tests for generic form component"
```
