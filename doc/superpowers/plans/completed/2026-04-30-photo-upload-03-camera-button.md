# Task 3: Add camera button to ChallengeCard

**Part of:** [Photo Upload](2026-04-30-photo-upload.md)
**Depends on:** [Task 2 — Create Worker script](2026-04-30-photo-upload-02-worker-script.md)
...
- [ ] **Step 1: Add failing tests for the four upload states**

Append the following `describe` block to the bottom of `src/test/ChallengeCard.test.jsx` (after the last existing `test(...)` call, before the end of the file). Also add `waitFor` and `fireEvent` to the existing `@testing-library/react` import at the top of the file.
...
- [ ] **Step 2: Run tests to confirm only the new ones fail**

```bash
npm run test:run
```

Expected: 4 new tests FAIL (camera button not yet added). All pre-existing tests still pass.

- [ ] **Step 3: Implement the camera button in ChallengeCard**

Replace the full contents of `src/components/ChallengeCard.jsx` with:
...
function getImageUrl(imageName) {
  switch (imageName) {
    case "alireza-parpaei-den-haag-binnenhof-unsplash.jpg":
      return new URL(
        "../data/img/alireza-parpaei-den-haag-binnenhof-unsplash.jpg",
        import.meta.url,
      ).href;
    case "rafael-ishkhanyan-den-haag-peace-palace-unsplash.jpg":
      return new URL(
        "../data/img/rafael-ishkhanyan-den-haag-peace-palace-unsplash.jpg",
        import.meta.url,
      ).href;
    case "den-haag-het-plein.jpg":
      return new URL("../data/img/den-haag-het-plein.jpg", import.meta.url)
        .href;
    default:
      return null;
  }
  ...
  const titleCard = (
    ...
    >
      ...
      <div>
        ...
        {location.name?.value && (
          <div
            style={{ fontSize: 13, color: theme.textSecondary, marginTop: 3 }}
          >
            {location.name.value}
          </div>
        )}
  ...
  return (
    <div style={{ background: theme.background }}>
      {hasHero ? (
        ...
        <div style={{ margin: 16 }}>{titleCard}</div>
      )}
      ...
      <div style={{ padding: 16, borderBottom: `1px solid ${theme.border}` }}>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          />
          {uploadState === "success" ? (
            <div style={{ fontSize: 13, color: "#2d7a2d", fontWeight: 600 }}>
              ✓ Photo submitted
            </div>
          ) : (
            <>
              <button
                ...
                style={{
                  ...
                  padding: "10px 0",
                  background:
                    uploadState === "uploading" ? theme.surface : "#002868",
                  ...
                  fontWeight: 600,
                  cursor:
                    uploadState === "uploading" ? "not-allowed" : "pointer",
                }}
              >
              ...
              {uploadState === "error" && (
                <div style={{ fontSize: 11, color: "#BF0A30", marginTop: 4 }}>
...
Expected: all tests pass, including the 4 new upload state tests. Total passing count increases by 4.
...
Open a location card. Confirm the `📷 Submit photo proof` button appears below the challenge description. On a phone (or using browser devtools mobile emulation), tap the button — it should open the camera or file picker.

> The upload itself will 404 in Vite dev mode (no Worker). Use `npm run preview` to test the full upload flow locally with wrangler's simulated R2.

- [ ] **Step 6: Commit**

```bash
git add src/components/ChallengeCard.jsx src/test/ChallengeCard.test.jsx
git commit -m "feat: add camera button and photo upload to ChallengeCard"
```
