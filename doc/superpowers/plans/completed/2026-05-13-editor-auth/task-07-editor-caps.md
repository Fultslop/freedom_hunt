# Task 07: Editor Capability Check + User Management

**Files:**
- Modify: `src/worker/routes/editorRoutes.ts`
- Modify: `src/test/worker.test.ts`

Replace the local `requireAdmin` function (which checks `authPayload?.isAdmin`) with `requireEditorCap` — a D1-backed check that verifies the user has `editor` or `organizer` capability for the project. The KV admin path (participant token with `isAdmin: true`) remains as a backward-compat fallback. Also adds `GET /editor/:project/users` and `POST /editor/:project/users/:userId/revoke`.

---

- [ ] **Step 1: Check existing tests that use `isAdmin` to understand what needs updating**

Run: `npm run test:run -- src/test/worker.test.ts`

Note which tests mock `isAdmin` in the token — those will need to change to use either a KV-admin participant token or a user token with D1 caps. The test mock setup in `worker.test.ts` uses `isAdmin: true` in the payload. That path (participant token + `isAdmin`) is the fallback, so those tests still work without changes — `requireEditorCap` allows it.

---

- [ ] **Step 2: Replace `src/worker/routes/editorRoutes.ts`**
...
The existing `worker.test.ts` mocks use a participant token with `isAdmin: true`. The new `requireEditorCap` allows participant tokens with `isAdmin: true` as a fallback, so those tests should still pass without modification.
...
```bash
git add src/worker/routes/editorRoutes.ts
git commit -m "feat: replace isAdmin check with D1 capability check in editor routes; add user list and revoke endpoints"
```
