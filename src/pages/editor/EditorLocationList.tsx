...
export default function EditorLocationList() {
  ...
  const syncPending = useCallback(() => {
    ...
    const numbers = items
      .map((p) => (p as { prUrl?: string }).prUrl?.match(/\/pull\/(\d+)/)?.[1])
      .filter(Boolean);
    ...
    fetch(`/editor/pr-status?numbers=${numbers.join(",")}`)
      ...
      .then((data) => {
        ...
        if (!typed.ok || !typed.statuses) return;
        const statuses = typed.statuses;
        let changed = false;
        items.forEach((p) => {
          const n = (p as { prUrl?: string }).prUrl?.match(/\/pull\/(\d+)/)?.[1];
          if (n && statuses[n] === "closed") {
  ...
  return (
    <div className="loc-list">
      <div className="loc-list__toolbar">
        ...
        <button
          className="loc-list__refresh-btn"
          onClick={() => {
            fetchLocations();
            syncPending();
          }}
