...
- **Token sync** — `App.svelte` syncs the JS theme object to `--color-*` CSS custom properties on `<html>` on every theme change via a `$effect` on `themeStore`.
- **Reactivity uses Svelte 5 runes:** `$state`, `$derived`, `$effect`, `$props`. Do not use Svelte 4 `$:` reactive statements.
